...
//Dropzone script
    ...
    $(document).ready(function() {
        ...
        var dataListView = $('.dataex-html5-selectors').DataTable( {
            ...
            responsive: false,
            ajax: {
                url: "{{ url('master/data/village/get') }}",
                type: 'GET',
            ...
            $.ajax({
                ...
                success:(function(data){
                    ...
                    for (let i = 0; i < (data.list_district).length; ++i) {
                        prop2 = (data.detail.district_id == data.list_district[i].id) ? 'selected="selected"' : '';
                        str2 += '<option value="'+data.list_district[i].id+'" '+prop2+'>'+data.list_district[i].name+'</option>';
                    }
                    $("#"+action+"-district").html(str2);
                    ...
                    for (let i = 0; i < (data.list_regency).length; ++i) {
                        prop1 = (data.single_district.regency_id == data.list_regency[i].id) ? 'selected="selected"' : '';
                        str1 += '<option value="'+data.list_regency[i].id+'" '+prop1+'>'+data.list_regency[i].name+'</option>';
                        
                        if (data.single_district.regency_id == data.list_regency[i].id) {
        ...
        $(document).on('change',"#add-province, #edit-province", function () {
            ...
            $.ajax({
                url: 'village/'+province_id+'/detailRegency',
                type:"GET",
                success:(function(data){ 
                    let str1 = '<option value=""></option>';
        ...
        $(document).on('change',"#add-regency, #edit-regency", function () {
            ...
            $.ajax({
                url: 'village/'+regency_id+'/detailDistrict',
                type:"GET",
                success:(function(data){ 
                    let str1 = '<option value=""></option>';
                    for (let i = 0; i < (data.detail).length; ++i) {
        ...
        $(document).on('change',"#add-district, #edit-district", function () {
            let district_id = '';
            let action = '';
            
            if($(this).attr("id") == "add-district"){
                district_id = $('#add-district').val();
                action = 'add';
            }else{
                district_id = $('#edit-district').val();
                action = 'edit';
            }

            $.ajax({
                url: 'village/'+district_id+'/detailVillage',
                type:"GET",
                success:(function(data){ 
                    let i = 1;                              
            ...
            let str =   '<!-- '+action+' form : BEGIN -->\
                        <div class="col-sm-12 data-field-col">\
                            ...
                            <div class="form-group">\
                                ...
                                <select class="select2 js-example-placeholder-single form-control" id="'+action+'-province"></select>\
                            </div>\
                            <div class="form-group">\
                                <label for="'+action+'-regency"><b>Regency</b></label>\
                                <select class="select2 js-example-placeholder-single form-control" id="'+action+'-regency"></select>\
                            </div>\
                            <div class="form-group">\
                                <label for="'+action+'-district"><b>District</b></label>\
                                <select class="select2 js-example-placeholder-single form-control" id="'+action+'-district"></select>\
                            </div>\
                            <div class="form-group">\
