...
//Dropzone script
    ...
    $(document).ready(function() {
        ...
        var dataListView = $('.dataex-html5-selectors').DataTable( {
            ...
            responsive: false,
            ajax: {
                url: "{{ url('master/data/province/get') }}",
                type: 'GET',
        ...
        $(".dt-buttons").on('click', function(event){    
            $.ajax({
                url: 'province/detailAdd',
                type:"GET",
                success:(function(data){
                    let action = 'add';
                    $("#add_body").html(createForm(action));
                    // BEGIN : val
                    let prop = 1, str = data.cek_province.id + prop;
                    $("#"+action+"-id").val(str);
                    // END : val
                }),error:function(xhr,status,error) {
                    Swal.fire({
